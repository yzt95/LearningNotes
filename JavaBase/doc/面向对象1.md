...
2. **成员变量**
    ...
    * 根据是否使用 static 修饰成员变量分为静态变量和实例变量
        * 非静态的实例变量储存在堆空间的对象中，在创建对象的时候实例化，存储在堆内存中
        * 类变量储存在方法区，随着类的加载而加载，存储在方法区
...
### 方法重载 overload
在同一个类中，允许一个以上的同名方法，只要它们的**参数个数**或**参数类型**不同，与方法的权限修饰符、返回值类型、形参变量名、方法体没有关系。但是，只有返回值不同的方法不构成重载。
```java
// 方法重载
...
* 如果参数是基本数据类型，方法体内是对该基本数据类型的数据进行操作，由于原始内容和副本都是存储实际值，并且是在不同的栈区，因此形参的操作，不影响原始内容
* 如果参数是引用数据类型，方法体内是对引用类型的数据进行操作，分两种情况
    * 一种是形参和实参保持指向同一个对象地址，则形参的操作，会影响实参指向的对象的内容
    * 一种是形参被改动指向新的对象（如重新赋值引用），则形参的操作，不会影响实参指向的对象的内容，也不会影响实参所保存的地址值
...
2. 对每个值属性提供对外的公共方法访问，如创建 get/set（取值和赋值）方法，用于对私有属性的访问
3. 在 get/set 方法里加入属性的控制语句，例如可以加一个判断语句，对于非法输入给予否定
```java
public class People {
...
// get和set方法需要程序员自己定义
    ...
    //get 方法命名是get关键字加属性名（属性名首字母大写）
    //get 方法一般是为了得到属性值
        return height;
    }

// 同理设置set方法
// set 方法命名是set关键字加属性名（首字母大写）
// set 方法一般是给属性值赋值，所以有一个参数
...
### 构造器的使用
* 每个类都至少有一个构造器。如果没有显式地为类定义构造器，Java 编译器将会为该类提供一个默认的空参构造器。一旦显式的定义了类的构造器之后，系统就不再提供默认的空参构造器
* 定义构造器的格式
```java
权限修饰符 类名(形参列表){}
...
```
* 通过方法重载，一个类可以有多个构造器
* 构造器若用 private 修饰，则不可使用该构造方法创建对象，但在本类的其他构造方法中仍然可以调用
* 构造器中使用 this 关键字即可调用其他构造器
```java
public class People {
    //属性（成员变量）有什么
    double height;     //身高
...
* 在方法中：使用 t**his.属性** 操作当前对象的属性，**this.方法** 调用当前对象的方法。一般情况下可以省略 this. ，但是当成员变量和局部变量（例如方法的形参）间发生冲突时，必须在属性名前面前室添加 this.
```java
...
* 包采用了树形目录的存储方式。同一个包中的类的名字是不同的，不同的包中的类的名字是可以相同的，当同时调用两个不同包中相同类名的类时，应该加上包名加以区别。因此，包可以避免类名或接口名冲突
* 包也是对类的访问权限的一种限定
* 包名全部小写，见名知意
* 包名中每 . 一次就代表一层文件目录
```java
...
* 类或接口如果在 java.lang 包下或本包内定义，使用时可以省略 import
