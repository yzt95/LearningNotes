...
* 由 CPU 原生提供的整型最大范围是 64 位 long 型整数，使用long型整数可以直接通过CPU指令进行计算，速度非常快。
* 如果需要的整数大小超过了 long 的范围，可以使用 BigInteger，java.math.BigInteger 可以表示不可变的任意精度的整数，其内部用一个int[]数组来模拟一个非常大的整数
